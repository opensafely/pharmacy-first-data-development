...
df_consultation_med_counts <- read_csv(
  here(output_dir, "data_development", "consultation_med_counts_measures.csv"),
...
df_filtered <- read_csv(
  (here("lib", "validation", "data", "eps_erd_prescribing_2024_feb.csv"))
)
...
code_descriptions <- here("codelists", codelist_file_names) |>
...
df_med_count_results <- df_consultation_med_counts |>
  select(numerator, code = dmd_code, pharmacy_first_med) |>
  left_join(code_descriptions, by = "code") |>
  filter(numerator > 0) |>
    group_by(code, pharmacy_first_med, term) |>
    summarise(
      count = sum(numerator, na.rm = TRUE),
      .groups = 'drop') |>
    arrange(desc(code))

non_pf_med_dict  <- tribble(
  ~code, ~term,
...
df_pf_med_counts <- df_med_count_results %>% 
  filter(pharmacy_first_med == TRUE) %>%
  arrange(desc(count))

df_nonpf_med_counts <- df_med_count_results %>% 
  filter(pharmacy_first_med == FALSE) %>%
  arrange(desc(count))

df_nonpf_med_counts <- df_nonpf_med_counts %>%
  left_join(non_pf_med_dict, by = "code") %>%
  mutate(term = coalesce(term.x, term.y)) %>%
  select(-term.x, -term.y)

df_pf_med_counts <- head(df_pf_med_counts,10)
df_nonpf_med_counts <- head(df_nonpf_med_counts,10)

tab_pf_med_counts <- df_pf_med_counts %>%
  select(term, count) %>%
  gt() %>%
  tab_header(
    title = "Top Ten Pharmacy First Medication",
    subtitle = "Timeframe: 1st Feb 2024 to 31st July 2024"
  ) %>%
  cols_label(
    term = "Medication Description",
    count = "Count"
  ) %>%
  fmt_number(
    columns = everything(),
...
tab_nonpf_med_counts <- df_nonpf_med_counts %>%
  select(term, count) %>%
  gt() %>%
  tab_header(
    title = "Top Ten Non-Pharmacy First Medication",
    subtitle = "Timeframe: 1st Feb 2024 to 31st July 2024"
  ) %>%
  cols_label(
    term = "Medication Description",
